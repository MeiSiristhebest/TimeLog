...
drop
policy if exists "family_members_select_own" on public.family_members;
create
policy "family_members_select_own"
...
drop
policy if exists "family_members_insert_admin" on public.family_members;
create
policy "family_members_insert_admin"
  on public.family_members
  for insert
  to authenticated
...
drop
policy if exists "family_members_update_admin" on public.family_members;
create
policy "family_members_update_admin"
  on public.family_members
  for
update
    to authenticated
...
create
or replace function public.create_family_invite(p_email text)
...
:= 'admin';
  ...
  if
inviter_role is null or inviter_role <> 'admin' then
...
values (admin_family_id, lower(trim(p_email)), 'member', 'pending', token, auth.uid(),
        now()) on conflict (family_id, email) do
update
...
-- RPC: accept invite
create
or replace function public.accept_family_invite(p_token text)
returns void
language plpgsql
security definer
set search_path = public, auth
