...
(
    ...
    family_user_id uuid not null references auth.users
(
    id
) on delete cascade,
    device_name text,
...
drop
policy if exists "devices_select_own" on public.devices;
create
policy "devices_select_own"
  on public.devices
  for
select
    to authenticated
...
drop
policy if exists "device_codes_select_own" on public.device_codes;
create
policy "device_codes_select_own"
...
create
or replace function public.list_family_devices()
returns table(
  id uuid,
  device_name text,
...
create
or replace function public.revoke_device(p_device_id uuid)
returns void
language plpgsql
security definer
set search_path = public, auth
