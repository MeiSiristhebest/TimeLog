...
CREATE
OR REPLACE FUNCTION notify_on_comment()
RETURNS TRIGGER AS $$
DECLARE
story_owner_id UUID;
BEGIN
...
IF
story_owner_id IS NOT NULL AND story_owner_id != NEW.user_id THEN
...
CREATE
OR REPLACE FUNCTION notify_on_reaction()
RETURNS TRIGGER AS $$
DECLARE
story_owner_id UUID;
BEGIN
...
COMMENT
ON FUNCTION notify_on_comment() IS 'Triggers notification grouping when new comment is posted';
COMMENT
ON FUNCTION notify_on_reaction() IS 'Triggers notification grouping when story is reacted to';
