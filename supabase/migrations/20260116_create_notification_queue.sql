...
DROP POLICY IF EXISTS "users_view_own_queue" ON notification_queue;
CREATE POLICY "users_view_own_queue"
  ON notification_queue FOR SELECT
  USING (user_id = auth.uid());

DROP POLICY IF EXISTS "service_manage_queue" ON notification_queue;
...
DROP POLICY IF EXISTS "users_manage_own_settings" ON user_notification_settings;
...
COMMENT ON TABLE user_notification_settings IS 'User preferences for notification delivery';
COMMENT ON COLUMN user_notification_settings.quiet_hours_start IS 'Start time for quiet hours (e.g., 21:00)';
COMMENT ON COLUMN user_notification_settings.quiet_hours_end IS 'End time for quiet hours (e.g., 09:00)';
COMMENT ON COLUMN user_notification_settings.time_zone IS 'IANA time zone (e.g., Asia/Shanghai)';
