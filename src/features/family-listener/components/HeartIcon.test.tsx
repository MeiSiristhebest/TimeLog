...
describe('HeartIcon', () => {
  ...
  it('should render without crashing', () => {
    const { getByRole } = render(<HeartIcon isLiked={false} onToggle={mockOnToggle} />);

    expect(getByRole('button')).toBeTruthy();
  });

  it('should have correct accessibility label when not liked', () => {
    const { getByLabelText } = render(<HeartIcon isLiked={false} onToggle={mockOnToggle} />);

    expect(getByLabelText('Like story')).toBeTruthy();
  });

  it('should have correct accessibility label when liked', () => {
    const { getByLabelText } = render(<HeartIcon isLiked={true} onToggle={mockOnToggle} />);
  ...
  it('should respect custom size prop', () => {
    const { getByRole } = render(<HeartIcon isLiked={false} onToggle={mockOnToggle} size={48} />);
