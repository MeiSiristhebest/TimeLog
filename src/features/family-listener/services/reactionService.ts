...
export async function addReaction(
    ...
    syncReactionToCloud(reaction).catch(err => {
        console.error('[reactionService] Failed to sync reaction:', err);
...
export async function removeReaction(
    ...
    try {
        ...
        if (error) {
            console.error('[reactionService] Failed to delete from cloud:', error);
        }
    } catch (err) {
        console.error('[reactionService] Cloud delete failed:', err);
    }
}
...
export async function getReactionCount(storyId: string): Promise<number> {
    try {
        ...
        if (error) {
            console.error('[reactionService] Failed to get reaction count:', error);
            return 0;
        }

        return count ?? 0;
    } catch (err) {
        console.error('[reactionService] getReactionCount failed:', err);
        return 0;
    }
}
...
export async function syncPendingReactions(): Promise<void> {
    ...
    for (const reaction of pendingReactions) {
        try {
            ...
            console.error(`[reactionService] Failed to sync reaction ${reaction.id}:`, err);
        }
    }
}
