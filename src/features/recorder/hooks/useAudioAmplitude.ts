...
type AudioAmplitudeState = {
  currentAmplitude: SharedValue<number>;
  updateAmplitude: (meteringDb: number) => void;
};
...
export function useAudioAmplitude(): AudioAmplitudeState {
  ...
  const updateAmplitude = useCallback(
    (meteringDb: number) => {
      ...
      // Range adjusted to be more sensitive
      const minDb = -80;
      const maxDb = -10; // slightly below 0 to avoid constant clipping
