...
export async function getRecommendedTopics(seniorUserId: string): Promise<Topic[]> {
    // 1. Fetch unanswered family questions
    const familyQuestions = await getUnansweredQuestions(seniorUserId);

    // 2. Load inspiration library
    ...
    const familyTopics: Topic[] = await Promise.all(
        familyQuestions.map(async (fq) => {
            ...
            try {
                ...
                console.error('Failed to fetch family member name:', error);
            }
...
export async function markQuestionAsAnswered(
    ...
    try {
        ...
        console.error('[topicService] Failed to create activity event:', error);
