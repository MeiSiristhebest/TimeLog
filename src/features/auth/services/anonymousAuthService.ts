...
import {
  getLocalProfile,
  updateLocalProfile,
} from '@/features/settings/services/localProfileService';
  ...
  try {
    ...
    if (error) {
      ...
      if (error.code === '23505' || error.code === '42501') {
        devLog.info(
          '[anonymousAuthService] Profile already exists/handled by trigger (safe to ignore)'
        );
        return;
      }
      throw error;
...
export async function upgradeAnonymousAccount(
  ...
  try {
    ...
    devLog.info(
      '[anonymousAuthService] Session refreshed successfully after upgrade:',
      refreshData.user?.id
    );
  ...
  // Automatically enable Cloud AI processing and enqueue any local offline recordings
  try {
    setCloudAiEnabled(true);
    await syncQueueService.reEnqueueOfflineRecordings();
    devLog.info('[anonymousAuthService] Cloud AI enabled and offline recordings enqueued');
  } catch (error) {
    devLog.warn(
      '[anonymousAuthService] Failed to enqueue offline recordings during upgrade:',
      error
    );
  }
