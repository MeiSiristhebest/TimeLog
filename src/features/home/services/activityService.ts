...
export async function getUnreadActivities(userId: string): Promise<Activity[]> {
    try {
        ...
        const localActivities = await db
            ...
            .where(
                and(
                    eq(activityEvents.targetUserId, userId),
                    isNull(activityEvents.readAt)
                )
            )
        ...
        console.error('[activityService] Error fetching unread activities:', error);
        return [];
    }
}
...
export async function getUnreadCount(userId: string): Promise<number> {
    try {
        ...
        console.error('[activityService] Error getting unread count:', error);
        return 0;
    }
}
